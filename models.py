...
class PoliceOfficer(Base):
    ...
    managers     = relationship("ManagementHierarchy",
                                foreign_keys="ManagementHierarchy.Subordinate_ID",
                                back_populates="subordinate")
    subordinates = relationship("ManagementHierarchy",
                                foreign_keys="ManagementHierarchy.Manager_ID",
...
class Accommodation(Base):
    ...
    pilgrim_accommodations = relationship("PilgrimAccommodation",
...
class Vendor(Base):
    ...
    License_Status  = Column(SAEnum(LicenseStatusEnum, name="license_status_enum"),
                             nullable=True)
...
class Transportation(Base):
    ...
    pilgrim_transportation = relationship("PilgrimTransportation",
...
class LostAndFound(Base):
    ...
    Availability        = Column(Boolean)
    Claim_Status        = Column(SAEnum(ClaimStatusEnum, name="claim_status_enum"),
                                 nullable=True)
    Location            = Column(String(255))
...
class IncidentReport(Base):
    ...
    Location            = Column(String(255), nullable=False)
    Reported_By         = Column(String(100))
    Status              = Column(SAEnum(IncidentStatusEnum, name="incident_status_enum"),
                                 nullable=True)
...
class ManagementHierarchy(Base):
    ...
    manager     = relationship("PoliceOfficer", foreign_keys=[Manager_ID],
                               back_populates="subordinates")
    subordinate = relationship("PoliceOfficer", foreign_keys=[Subordinate_ID],
